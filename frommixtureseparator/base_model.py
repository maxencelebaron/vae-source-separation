...
class BetaVAE(VAE):
    ...
    def fit(self, data_loader, num_epochs=20, lr=1e-3):
        """
        Implémentation de fit() pour la Beta-VAE (inclut le coefficient beta).
        """
        # Assurez-vous que self.beta et self.device sont définis
        optimizer = optim.Adam(self.parameters(), lr=lr)
        reconstruction_loss_fn = nn.MSELoss(reduction='sum')
        ...
        self.train()
        print(f"Starting BetaVAE training for {num_epochs} epochs on device: {self.device}")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for batch_idx, (mixed, signal, phase) in enumerate(data_loader):
                ...
                # Divergence KL
                # Utilise mu et log_var pour calculer la distance à N(0, I)
                kl_div = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())

                # Perte totale : Recon + Beta * KL
            ...
            # Calcul de la perte moyenne par échantillon
            avg_loss = total_loss / len(data_loader.dataset)
            loss_track.append(avg_loss)
            print(f"Epoch {epoch}/{num_epochs}, Avg Loss: {avg_loss:.4f} (Beta={self.beta})")
...
class Encoder_ITL_AE(nn.Module):
    ...
    def __init__(
        ...
        latent_dim: int = 32,   # dimension espace latent
    ):
...
class ITL_AE(nn.Module):
    ...
    def predict(self, test_loader):
        ...
        for batch_idx, (mixed, signal, phase) in enumerate(test_loader):
            x_mixte = mixed.to(self.device)   # Mixture (Input)
            y_cible = signal.to(self.device)  # Signal propre (Target)

            # Forward pass : ITL-AE (retourne z, x_hat)
            z, x_hat = self.forward(x_mixte)

            # Stockage des résultats sur CPU
            reconstructed_sources.append(x_hat.cpu())
            true_sources.append(y_cible.cpu())
            mixed_sources.append(x_mixte.cpu())

            # AJOUT : Stockage de la phase pour la reconstruction audio
            phase_sources.append(phase.cpu())
