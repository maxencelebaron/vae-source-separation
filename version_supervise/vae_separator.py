...
class ITL_Separator:
    ...
    séparation par VAE mais aucun prior gaussien n'est utilisé
    (contrairement au VAE).
    """
    def __init__(self, itl_s1, itl_s2, device):
        ...
        # On utilise le même lambda que pour la régularisation CS-Div
    ...
    def separate(self, mix_log_mag, num_steps=300, lr=0.05):
        """
        ...
        for step in range(num_steps):
            ...
            # Conversion log -> linéaire (avec relu et offset pour éviter log(0))
            lin_s1_hat = torch.relu(torch.expm1(log_s1_hat)) + 1e-8
            lin_s2_hat = torch.relu(torch.expm1(log_s2_hat)) + 1e-8

            # Reconstruction de la magnitude du mélange
            lin_mix_hat = lin_s1_hat + lin_s2_hat
            log_mix_hat = torch.log1p(lin_mix_hat)  # log(1 + lin_mix_hat)

            #  2. PERTE DE RECONSTRUCTION DU MÉLANGE (Mix Loss)
            reconstruction_loss = mse(log_mix_hat, mix_log_mag)

            # 3. PERTE DE BOUCLE (CYCLE LOSS) - NOUVELLE CONTRAINTE
            # Encodage de la reconstruction de la source dans son propre encodeur
            # Ceci garantit que z_i se situe bien sur le manifold appris
