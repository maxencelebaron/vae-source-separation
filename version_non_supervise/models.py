...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        # x est [B,1,F,T]
        # B = Batch size
        # C = Channels (1 ici, car spectrogramme mono)
        # F = Frequency bins (257, nombre de fréquences)
        # T = Time frames
        B, C, F, T = x.shape
...
class VAE(nn.Module):
    ...
    def predict(self, test_loader):
        ...
        for batch_data in test_loader:
            ...
            x_hat, _, _, _ = self.forward(x_true)
...
class BetaVAE(VAE):
    ...
    def fit(self, data_loader, num_epochs=20, lr=1e-3):
        ...
        loss_track = []
        # Track séparé pour reconstruction
        recon_track = []
        # Track séparé pour KL
        kl_track = []

        self.train()
        print(f"Starting β-VAE training (β={self.beta}) for {num_epochs} epochs on device: {self.device}")
...
class ITL_AE(nn.Module):
    ...
    def loss_function(self, X, X_hat, z, prior_samples=None):
        """
        Calcul de la loss ITL-AE
        X : [B,1,F,T]
        X_hat : [B,1,F,T]
        z : [B,T,latent_dim]
        prior_samples : [B,T,latent_dim], sinon prior N(0,I)
        """
        ...
        # cs_div = log( (Vq * Vp) / Vc² )
        eps = 1e-12
        cs_div = torch.log((Vq * Vp + eps) / (Vc * Vc + eps))

        # 4) Total loss
        loss = recon_loss + self.regul_param * cs_div

        return loss, recon_loss, cs_div
    ...
    def fit(self, data_loader, num_epochs=20, lr=1e-3, prior_samples=None):
        ...
        loss_track = []
        # Track séparé pour reconstruction (Recon)
        recon_track = []
        # Track séparé pour la Divergence de Cauchy-Schwarz (CS-Div)
        cs_div_track = []
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for batch_data in data_loader:
                ...
                # Calcul de la Loss
                loss, recon_loss, cs_div = self.loss_function(
                    x, x_hat, z, prior_samples=prior_samples
                )
            ...
            avg_loss = total_loss / len(data_loader.dataset)
            avg_recon = total_recon / len(data_loader.dataset)
            avg_cs_div = total_cs_div / len(data_loader.dataset)

            loss_track.append(avg_loss)
            recon_track.append(avg_recon)
            cs_div_track.append(avg_cs_div)

            print(
                f"Epoch {epoch}/{num_epochs} | "
                f"Loss: {avg_loss:.4f} | "
                f"Recon: {avg_recon:.4f} | "
                f"CS-Div: {avg_cs_div:.4f}"
            )
        ...
        return {
            'total_loss': loss_track,
            'recon_loss': recon_track,
            'cs_div_loss': cs_div_track
        }

    @torch.no_grad()
    def predict(self, test_loader):
        # Met le modèle en mode évaluation (désactive dropout, etc. si utilisé)
        self.eval()
